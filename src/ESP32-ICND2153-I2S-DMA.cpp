...
void IRAM_ATTR i2s_isr_cb(){
  led_matrix.i2s_isr_cb_dma2153();
}
...
bool ICND2153_I2S_DMA::begin()
{
  ...
  configureDMA(PIN_R1, PIN_G1, PIN_B1, PIN_R2, PIN_G2, PIN_B2, PIN_A, PIN_B, PIN_C, PIN_LAT, PIN_OE, PIN_CLK); //DMA and I2S configuration and setup

  //showDMABuffer(); // show backbuf_id of 0

  #if SERIAL_DEBUG 
    if (!everything_OK)    
          Serial.println("ICND2153_I2S_DMA::begin() failed.");
  #endif
...
bool ICND2153_I2S_DMA::allocateDMAmemory()
{
  ...
  */
   ...
   */
  {
    //1 is for header of frame(Vsync), frame_scan_num * 2 = colorbuffer_dma_descount + auxbuffer_dma_descount, and double  
...
void ICND2153_I2S_DMA::configureDMA(int r1_pin, int  g1_pin, int  b1_pin, int  r2_pin, int  g2_pin, int  b2_pin, int  a_pin, int   b_pin, int  c_pin, int  lat_pin, int   oe_pin, int clk_pin)
{
    ...
    if (desccount_a != current_dmadescriptor_offset){
      Serial.printf("configureDMA(): ERROR! Color buffer Expected descriptor count of %d != actual DMA descriptors of %d!\r\n", desccount_a, current_dmadescriptor_offset);		  
    }

    /*auxiliary buffer link descriptors*/
    lldesc_t *previous_dmadesc_b     = 0;
    current_dmadescriptor_offset = 0;
    link_dma_desc(&dmadesc_b[current_dmadescriptor_offset], previous_dmadesc_b, header_buffer, sizeof(uint16_t) * header_length);
    previous_dmadesc_b = &dmadesc_b[current_dmadescriptor_offset];
    current_dmadescriptor_offset++;
    for(int scan_id = 0; scan_id < frame_scan_num; scan_id++) {
      link_dma_desc(&dmadesc_b[current_dmadescriptor_offset], previous_dmadesc_b, aux_buffer, sizeof(uint16_t) * dma_unit_num);
      previous_dmadesc_b = &dmadesc_b[current_dmadescriptor_offset];
      current_dmadescriptor_offset++;
      link_dma_desc(&dmadesc_b[current_dmadescriptor_offset], previous_dmadesc_b, &aux_buffer[dma_unit_num], sizeof(uint16_t) * dma_unit_num);
      previous_dmadesc_b = &dmadesc_b[current_dmadescriptor_offset];
      current_dmadescriptor_offset++;
    }
    if (desccount_b != current_dmadescriptor_offset){
      Serial.printf("configureDMA(): ERROR! Auxiliary buffer Expected descriptor count of %d != actual DMA descriptors of %d!\r\n", desccount_b, current_dmadescriptor_offset);		  
    }
    #if SERIAL_DEBUG  

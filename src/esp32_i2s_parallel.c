...
void i2s_parallel_setup_without_malloc(i2s_dev_t *dev, const i2s_parallel_config_t *cfg) {
  ...
  if (dev == &I2S0) {
    ...
    if (cfg->bits == I2S_PARALLEL_BITS_32) {
      ...
      // printf("Setting up i2s parallel mode in %d bit mode -> https://www.esp32.com/viewtopic.php?f=17&t=3188 |
      // https://www.esp32.com/viewtopic.php?f=13&t=3256", 8);
      sig_data_base = I2S1O_DATA_OUT0_IDX;
    }
  ...
  // gpio_matrix_out(cfg->gpio_clk, sig_clk, true, false);
  ...
  // Enable "One datum will be written twice in LCD mode" - for some reason, if we don't do this in 8-bit mode, data is
  // updated on half-clocks not clocks
  ...
  // We ignore the possibility for fractional division here, clkspeed_hz must round up for a fractional clock speed,
  // must result in >= 2
  ...
  dev->out_link.addr  = ((uint32_t)(&st->dmadesc_b[0]));
